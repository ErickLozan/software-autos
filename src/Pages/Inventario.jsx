...
export const Inventario = () => {
  ...
  return (
    <div className='flex'>
      ...
      <Modal display={display1} title="Filtrar Información" subTitle='Inventario'>
      ...
      <Modal display={display2} title='Eliminar registro' subTitle='001250923'>
        ...
        <div className="mb-5 text-center">
          <h3 className="text-2xl">¿Seguro que desea eliminar este registro?</h3>
          <span className="text-gray-500">Se borrará toda la información referente a este vehículo.</span>  
        </div>

        <button onClick={() => setDisplay2('hidden')} className="p-2 rounded-md border border-red-400 bg-gradient-to-br from-red-500 to-red-300 cursor-pointer transition-all duration-300 hover:shadow-lg font-medium text-white w-full">Eliminar</button>
      </Modal>

      <div className='px-10 py-5 w-full bg-slate-50 flex flex-col gap-5'>
        <div className='py-4 text-transparent bg-clip-text bg-gradient-to-br from-orange-600 to-orange-300'><h2 className='text-8xl font-extrabold'>Inventario</h2></div>

        <div className='flex gap-5'>
          <button onClick={filtrarBtn} className='w-min px-14 py-2 bg-slate-200 border border-slate-600 hover:shadow-md transition-all duration-300'>Filtrar</button>
          <Link to="/nuevo-auto" className='w-min px-14 py-2 bg-white border border-slate-600 hover:shadow-md transition-all duration-300'>Nuevo<span className='opacity-0'>_</span>Auto</Link>
        </div>
        ...
        <table className='w-[80%]'>
          ...
          <tbody>
            {
              array.map(registro => (
                <tr key={registro} className='text-center border-b border-b-gray-400 bg-white'>
                  <td className='py-2'><Link to='/inventario/001250923' className='transition-all duration-300 hover:text-slate-500'>001250923</Link></td>
                  <td>NISSAN</td>
                  <td>SENTRA</td>
                  <td>2020</td>
                  <td>50,000</td>
                  ...
                  <td className='flex gap-3 py-2'>
                    ...
                    <button onClick={() => setDisplay2('flex')}><span className='material-symbols-outlined text-red-800'>delete</span></button>
                  </td>
                </tr>
              ))
            }
